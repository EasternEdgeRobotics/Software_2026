...
public:
  PilotListener() : Node("PilotListener")
  {
    ...
    current_bluestar_control_values.thrust.fill(0.0f);
    ...
    // Create a publisher on the bluestar_control_values topic
    bluestar_control_publisher = this->create_publisher<eer_interfaces::msg::BlueStarControl>("bluestar_control_values", 10);
...
private:
  ...
  rclcpp::Publisher<eer_interfaces::msg::BlueStarControl>::SharedPtr bluestar_control_publisher;
  rclcpp::TimerBase::SharedPtr software_to_board_communication_timer;

  std::array<std::mutex, 6> target_thrust_mutexes;
  std::mutex current_bluestar_control_values_mutex;
  ...
  eer_interfaces::msg::BlueStarControl current_bluestar_control_values;
  ...
  void pilot_listener_callback(eer_interfaces::msg::PilotInput::UniquePtr pilot_input)
  {
    ...
    {
      std::lock_guard<std::mutex> lock(current_bluestar_control_values_mutex);
    ...
    auto compute_thrust_value = [this](int thruster_index, const eer_interfaces::msg::PilotInput *pilot_input, std::array<float, 6> &target_thrust_values)
    {
      ...
      else if (single_thruster_configuration_mode)
      {
        ...
        if (target_thrust_values[thruster_index] > 0.0f && stronger_side_positive[thruster_index])
          target_thrust_values[thruster_index] = target_thrust_values[thruster_index] * thruster_stronger_side_attenuation_constant;
        else if (target_thrust_values[thruster_index] < 0.0f && !stronger_side_positive[thruster_index])
          target_thrust_values[thruster_index] = target_thrust_values[thruster_index] * thruster_stronger_side_attenuation_constant;
      }
      else
      {
        ...
        float thrust_value = ((pilot_input->surge * THRUSTER_CONFIG_MATRIX[thruster_index][SURGE] +
                               ...
                               pilot_input->yaw * THRUSTER_CONFIG_MATRIX[thruster_index][YAW]) /
                              100.0f) *
                             thruster_direction[thruster_index];

        if (thrust_value > 0.0f && stronger_side_positive[thruster_index])
          thrust_value = thrust_value * thruster_stronger_side_attenuation_constant;
        else if (thrust_value < 0.0f && !stronger_side_positive[thruster_index])
          thrust_value = thrust_value * thruster_stronger_side_attenuation_constant;
  ...
  void software_to_board_communication_timer_callback()
  {
    ...
    for (int thruster_index = 0; thruster_index < 6; thruster_index++)
    {
      ...
      std::scoped_lock lock(target_thrust_mutexes[thruster_index], current_bluestar_control_values_mutex);

      // Only update the thrust value of each thruster if the target value is different
      if (target_thrust_values[thruster_index] != current_bluestar_control_values.thrust[thruster_index])
      {
        float difference = target_thrust_values[thruster_index] - current_bluestar_control_values.thrust[thruster_index];

        if (abs(difference) > thruster_acceleration)
        {
          current_bluestar_control_values.thrust[thruster_index] += std::copysign(difference * thruster_acceleration, difference);
        }
        else
        {
          current_bluestar_control_values.thrust[thruster_index] = target_thrust_values[thruster_index];
        }
      }
    }

    // Publish the BlueStar control values
    {
      std::lock_guard<std::mutex> lock(current_bluestar_control_values_mutex);
      bluestar_control_publisher->publish(current_bluestar_control_values);
    }
  }
    ...
    auto result = bluestar_config_client->async_send_request(request,
                                                             [this](rclcpp::Client<eer_interfaces::srv::GetConfig>::SharedFuture future)
                                                             {
                                                               ...
                                                               try
                                                               {
                                                                 ...
                                                                 {
                                                                   for (size_t i = 0; i < current_bluestar_control_values.thruster_map.size(); i++)
                                                                   {
                                                                     if (i >= configuration_data["thruster_map"].size() || configuration_data["thruster_map"][i].is_null() || !configuration_data["thruster_map"][i].is_number_integer() || i >= configuration_data["reverse_thrusters"].size() || configuration_data["reverse_thrusters"][i].is_null() || !configuration_data["reverse_thrusters"][i].is_boolean() || i >= configuration_data["stronger_side_positive"].size() || configuration_data["stronger_side_positive"][i].is_null() || !configuration_data["stronger_side_positive"][i].is_boolean())
                                                                     {
                                                                       valid_thruster_configuration = false;
                                                                 ...
                                                                 if (valid_thruster_configuration)
                                                                 {
                                                                   for (size_t i = 0; i < current_bluestar_control_values.thruster_map.size(); i++)
                                                                   {
                                                                     current_bluestar_control_values.thruster_map[i] = static_cast<int8_t>(std::abs(configuration_data["thruster_map"][i].get<int>()));
